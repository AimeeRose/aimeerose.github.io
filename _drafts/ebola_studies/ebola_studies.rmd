...
enrollment_populations <- ggplot() +
  geom_point(data = ebola_studies, aes(x = Start.Date.f, y = Enrollment, color = Recruitment), size = 8, alpha = 0.5) +
  scale_y_log10(breaks = seq(200, 1000, 200)) +
  labs(x = 'Start date', y = 'Target enrollment') +
  ggtitle('Ebola studies target enrollment') + legend + title
...
qplot(factor(year), data = groups, geom="bar", fill=factor(sponsor)) +
  ...
  theme(legend.title = element_blank(), legend.key.size = unit(1.5, "cm"), legend.text = element_text(size = rel(0.6))) + title
```
